      ...
      */
      newPuzzle: function(words, settings) {
        ...
        var options = {
		  ...
		  alphabet:         opts.alphabet !== undefined ? opts.alphabet : LETTERS
        };
        ...
        if (options.fillBlanks) {
            ...
            if (typeof options.fillBlanks === 'function') {
				...
				console.log(options.alphabet);
				extraLetterGenerator = () => options.alphabet[Math.floor(Math.random() * options.alphabet.length)];
					/*if (options.alphabet!=''){
						extraLetterGenerator = () => options.alphabet[Math.floor(Math.random() * options.alphabet.length)];
					}
					else{
