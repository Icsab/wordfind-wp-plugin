...
(function (document, $, wordfind) {
  ...
  */
  ...
  /*  var drawPuzzle = function (el, puzzle) {
	...
	*/
	...
	var renderWords = function (el, wordList,quizList,quiz){
		let output='';
		...
		if(quiz ){
			...
			output+="<li class='word quiz'><strong>"+quizList[wd]+"  </strong><span class='hidden-hint'>(<span class='word_element'>"+wd+"</span>)</span><span class='show-hint' title='Reveal word'>+</span><span class='show-word' title='Solve word' data-word='"+wd+"'></span></li>";
			}
		}
		else
		{
			for(var i=0; i < wordList.length; i++){
			output+="<li class='word'><span class='word_element'>"+wordList[i]+"</span><span class='show-word' title='Solve word' data-word='"+wordList[i]+"'></span></li>";
			}
		}
		$(el).html(output);
	}
  ...
  */
  var WordFindGame = function (puzzleEl, options, savedPuzzle, savedWords,quizList) {
    ...
    */
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
		...
		$('#wordlist li .word_element').filter(function() {
			if($(this).text() === curWord){
				$(this).addClass('wordFound');
				$(this).parents('.word').addClass('wordFound');
			}
		});
    ...
    /* Constructor START */
    $('#wordlist li').removeClass('wordFound');
	 $('#wordlist li .word_element').removeClass('wordFound');

    // Class properties, game initial config:
   // wordList = getWords().sort();
   // 
	  
		  wordList = savedWords.sort();
    ...
    // Draw the grid and all of the words
    drawPuzzle(puzzleEl, puzzle);
	  
	renderWords("#wordlist",wordList,quizList,options.quiz);
    ...
    this.solve = function() {
      ...
      for( var i = 0, len = solution.length; i < len; i++) {
       ...
       // var wordEl = $('input.word[value="' + word + '"]');
		 ...
		 var wordEl= $('#wordlist li .word_element').filter(function() {
			if($(this).text() === word){
				return $(this);
			}
		});
        ...
        if (!wordEl.hasClass('wordFound')) {
		  ...
		  wordEl.parents('.word').addClass('wordFound');
        }
      }
    };
	  
	  this.solveWord =function(w){
		 var solution = wordfind.solve(puzzle, wordList).found;
		   for( var i = 0, len = solution.length; i < len; i++) {
			 if(w == solution[i].word){
        ...
        if (!wordEl.hasClass('wordFound')) {
          for (var j = 0, size = w.length; j < size; j++) {
            var nextPos = next(x, y, j);
            $('[x="' + nextPos.x + '"][y="' + nextPos.y + '"]').addClass('solved');
          }
  ...
  // Static method
  WordFindGame.insertWordBefore = function (el, word, hint) {
	let index=$('#words li.word').length+1
	  
	  $('<li class="word"><label for="hint-'+index+'" class="wf_floating hint"><span>Question #.'+index+'</span><input class="hint"  id="hint-'+index+'" value="'+(hint || '')+'" /></label><label class="wf_floating"for="word-'+index+'"><span>Word #.'+index+'</span><input class="word  id="word-'+index+'"  value="'+(word || '')+'"/></label> </li>').insertBefore(el);
	  
		//  $('<li><input class="word" value="' + (word || '') + '"><textarea class="hint">'+(hint || '')+'</textarea></li>').insertBefore(el);
